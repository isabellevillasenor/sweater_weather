...
describe BackgroundImage do
  it 'exists with attributes' do
    ...
    expect(background_image.credit).to have_key(:source)
    expect(background_image.credit[:source]).to eq('https://unsplash.com/')
    expect(background_image.credit).to have_key(:author)
    expect(background_image.credit[:author]).to eq('Andrew Coop')
    expect(background_image.credit).to have_key(:author_username)
    expect(background_image.credit[:author_username]).to eq('andrewcoop')
    expect(background_image.credit).to have_key(:logo)
    expect(background_image.credit[:logo]).to eq('https://unsplash.com/@andrewcoop')
  end
end